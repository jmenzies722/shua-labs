    ...
    <PageShell>
      ...
      <article className="section-pad border-b border-line">
        <div className="site-shell max-w-3xl">
          ...
          <time dateTime={entry.date} className="text-[12px] text-fg-subtle">
            {formatDateStamp(entry.date)}
          </time>
          <h1 className="display-section mt-3 mb-4">{entry.title}</h1>
          <p className="body-lg mb-8">{entry.summary}</p>

          <div className="space-y-4 border-t border-line pt-6 text-[15px] leading-relaxed text-fg-muted">
            {entry.body.map((para) => (
              <p key={para}>{para}</p>
            ))}
          </div>
          ...
          <div className="mt-8 flex flex-wrap gap-4 text-[13px]">
            {entry.github ? (
              <a href={entry.github} className="text-fg hover:underline" rel="noreferrer" target="_blank">
                GitHub →
              </a>
            ) : null}
